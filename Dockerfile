...
RUN mv /jdk-8u5-linux-x64.tar.gz /usr/local; cd /usr/local; tar zxvf jdk-8u5-linux-x64.tar.gz ; rm -f jdk-8u5-linux-x64.tar.gz

RUN wget -q --no-cookies --no-check-certificate "http://download.java.net/glassfish/4.0/release/glassfish-4.0.zip"

RUN mv /glassfish-4.0.zip /usr/local; cd /usr/local; unzip glassfish-4.0.zip ; rm -f glassfish-4.0.zip
...
RUN asadmin --user admin --passwordfile /tmp/password_1.txt change-admin-password --domain_name domain1
RUN asadmin start-domain
RUN asadmin --user admin --passwordfile /tmp/password_2.txt enable-secure-admin
